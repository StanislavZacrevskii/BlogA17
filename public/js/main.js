...
/*
client.getUserCoursesAndModules(function (error, data) //Доступаемся до курсов и модулей
{
    ...
    data.courses.forEach(function(element)
    {
        ...
        client.getCourseInfo(id, function(error, data)
        {
            ...
            var list = [data.what_you_learn_ua];

            var elements = document.querySelectorAll('#about_courses');

            elements.innerHTML = list;//вставити
        });
            ...
            for(var i = 0; i<modules.length; i++ )   //MODULES
            {
                client.getModuleInfo(modules[i].id, function(error, my_data)
                {
                    if(my_data.title_ua !== "")
                    {
                        ...
                        {
                            var my_container = document.getElementById("modules");

                            my_container.innerHTML += "<hr>" + my_data.title_ua.toUpperCase() + "<hr>" + "<br>";
                            data.forEach(function(item){
                                my_container.innerHTML += item.title + "<br>";
                            });

                            my_container.innerHTML += "<br>";
                        });
                    }
                });
